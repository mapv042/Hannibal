"""Celery tasks for scheduling notices: patient notices for dashboard changes,
and the Rule 13 report that a requested reschedule ended in a cancellation."""
...
def enqueue_patient_cancellation_notice(appointment_id: UUID) -> None:
    ...
    dispatch(
        notify_patient_cancellation_task,
        [str(appointment_id)],
        countdown=NOTIFY_COUNTDOWN_SECONDS,
        event="patient_cancellation_notice_enqueued",
...
def enqueue_patient_reschedule_notice(appointment_id: UUID) -> None:
    ...
    dispatch(
        notify_patient_reschedule_task,
        [str(appointment_id)],
        countdown=NOTIFY_COUNTDOWN_SECONDS,
        event="patient_reschedule_notice_enqueued",
        appointment_id=str(appointment_id),
    )
...
def enqueue_abandoned_reschedule_notification(cancelled_appointment_id: UUID) -> None:
    ...
    dispatch(
        notify_doctor_abandoned_reschedule_task,
        [str(cancelled_appointment_id)],
        countdown=NOTIFY_COUNTDOWN_SECONDS,
        event="abandoned_reschedule_notification_enqueued",
