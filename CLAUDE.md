...
**When Google rejects the office's credentials** (`GoogleCalendarAuthError`: revoked grant, `invalid_grant`, 401/403 after a forced refresh — not timeouts/5xx, which still surface as "problema técnico"), scheduling **keeps going on the system's own agenda** (hours + appointments + blocks; only Google-only events are unseen), Google is not retried for 10 min, and the doctor gets a WhatsApp notice at most once a day (`doctor_calendar_disconnected` template out of window) — `google_calendar/connection.py`. The write audit skips its per-appointment calendar checks while disconnected. Reconnecting (OAuth callback) clears the state and backfills Google events for upcoming citas booked meanwhile.
